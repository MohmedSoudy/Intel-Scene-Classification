...
def Ensure_alphapet(Sequence):
    ...
    DNA_Ratio = ((Sequence.count('A') + Sequence.count('G') + Sequence.count('C') + Sequence.count('T') +
...
def Process_DF(DF):
    for i in range(1, len(DF.axes[0])):
        ...
        elif DF['alphabet'][i] == "ProteinAlphabet()":
            Write_fasta("Patents_Protein.fasta", DF['id'][i], DF['sequence'][i])
        else:
            Write_fasta("Patents_Others.fasta", DF['id'][i], DF['sequence'][i])
